...
This paper studies OPD through the lens of outcome alignment. Instead of treating every teacher-student logit difference as equally useful, we ask whether the direction of the teacher signal agrees with the final correctness of the sampled trajectory. For a candidate token, if the teacher assigns higher probability than the student and the trajectory is correct, the signal is outcome-aligned because it reinforces behavior appearing in a successful response. If the trajectory is incorrect, the aligned direction is reversed: teacher signals that reduce the likelihood of the sampled behavior are more consistent with the outcome. This gives a simple but informative partition of token candidates into outcome-aligned and outcome-anti-aligned groups.

We propose Outcome-Aligned Logit OPD (OAL-OPD), a family of OPD objectives that uses this partition to modulate dense teacher rewards. The basic hard variant keeps only token candidates whose teacher-student logit difference agrees with the final outcome. However, hard masking may discard useful information from anti-aligned tokens, especially when local and global credit assignment are noisy. We therefore introduce OAL-rank, a rank-calibrated variant that keeps aligned candidates at full strength and assigns anti-aligned candidates a small rank-based weight. This makes OAL-rank a conservative modification of OPD: it preserves the main dense teacher signal when it is outcome-consistent, while using a bounded auxiliary signal to avoid overcommitting to a binary token decision.

Our method is designed to answer two questions. First, can final correctness provide a useful calibration signal for token-level distillation? Second, when token-level teacher rewards conflict with the final outcome, should those tokens be removed entirely or retained with reduced influence? OAL-rank directly addresses the second question by replacing hard deletion with beta-capped rank weighting. This design also enables interpretable diagnostics, such as the fraction of aligned and anti-aligned candidates, their reward mass, and their retained supervision weights.
...
\begin{itemize}
    ...
    \item We introduce OAL-rank, which preserves aligned teacher supervision while softly down-weighting anti-aligned candidates using a bounded rank score.
...
The hard OAL objective keeps only outcome-aligned candidates:
\begin{equation}
w^{\mathrm{hard}}_{t,k}
=
{\bf 1}[s_{t,k}>m].
...
\end{equation}
This objective is easy to interpret, but it can be too aggressive. A token may be classified as anti-aligned because of local uncertainty, top-$K$ truncation, or noise in the final reward. Removing all such candidates may lose useful information.

\subsection{OAL-Rank}

OAL-rank keeps the main outcome-aligned supervision intact while softly retaining a bounded signal from anti-aligned candidates. For each response, we rank only candidates in ${\cal B}$ according to their outcome-aligned score $s_{t,k}$. Let $q_{t,k}\in[0,1]$ be the normalized rank of an anti-aligned candidate within that response. Candidates with relatively larger $s_{t,k}$ are closer to the decision boundary and receive larger rank values. We then define
\begin{equation}
w^{\mathrm{rank}}_{t,k}
=
\left\{
...
\end{equation}
Unlike a rank scheme that rescales all candidates, OAL-rank never down-weights outcome-aligned candidates. The ranking is applied only to the anti-aligned region, so $\beta$ has a clear meaning: it is the largest possible supervision strength assigned to candidates whose local teacher preference conflicts with the final outcome.

\subsection{Position-Level OAL-Rank}

We also consider a position-level version that aggregates candidate-level evidence before assigning a weight. For each position,
\begin{equation}
s_t
=
y \sum_{k=1}^{K}\Delta_{t,k}.
\end{equation}
The position weight is then
\begin{equation}
w^{\mathrm{pos}}_t
=
\left\{
...
\right.
\end{equation}
where $q_t$ is the normalized rank among anti-aligned positions in the same response. The position weight is broadcast to all candidates at that position:
\begin{equation}
A_{t,k}
=
