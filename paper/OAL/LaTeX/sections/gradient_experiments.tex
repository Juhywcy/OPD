...
\end{equation}
Cosine is averaged over $\mathcal J_X$ per audit batch rather than computed after averaging gradients.  Near-zero candidate gradients are not silently discarded: their rate $z_0^X$ is reported, while the common reference-valid count is $|\mathcal J|=\AuditN$.  We also report the batch-wise mean of $\lVert g^X_{j,A}\rVert/(\lVert g^{\mathrm{out}}_{j,B}\rVert+\epsilon)$ over all $j\in\mathcal J$, including candidate zeros, so that an apparent alignment gain cannot be attributed merely to shrinking the update toward zero.
