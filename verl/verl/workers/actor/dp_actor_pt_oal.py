...
from verl.trainer.ppo.core_algos_pt_oal import agg_loss, get_policy_loss_fn, kl_penalty
...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_distillation_reward(self, data: DataProto) -> DataProto:
        ...
        res_tensors = {}
        teacher_candidate_log_probs = None
        
        if strategy == "only_stu":
            kl_val = S_logp - T_on_S
            logit_delta_scores = -kl_val
            teacher_candidate_log_probs = T_on_S
            ...
            logit_delta_scores = -kl_val
            teacher_candidate_log_probs = T_logp
            ...
            kl_val = torch.where(valid_mask, kl_val, torch.zeros_like(kl_val))
            logit_delta_scores = -kl_val
            teacher_candidate_log_probs = T_logp_union
            norm_weights = compute_reward_weights(
                S_logp_union,
                T_logp_union,
                valid_mask,
        ...
        for _ in range(self.config.ppo_epochs):
            for batch_idx, mini_batch in enumerate(mini_batches):
                ...
                for micro_batch in micro_batches:
                    ...
                    if "odw_loss_mask" in model_inputs.keys():
                        odw_response_mask = response_mask * model_inputs["odw_loss_mask"].to(dtype=response_mask.dtype)
                        if odw_response_mask.sum() > 0:
                            response_mask = odw_response_mask
