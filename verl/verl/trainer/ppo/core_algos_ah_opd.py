...
def _ahopd_reliability_weights(token_level_rewards, response_mask, config, overlap_mask=None, teacher_entropy=None):
    ...
    if ah_cfg["soft_weighting"]:
        token_weights = ah_cfg["suffix_weight"] + (1.0 - ah_cfg["suffix_weight"]) * reliability
        token_weights = torch.where(
            prefix_mask > 0,
            token_weights,
            torch.full_like(token_weights, ah_cfg["suffix_weight"]),
        )
    else:
        token_weights = torch.where(
            prefix_mask > 0,
...
def compute_adaptive_horizon_token_reward_advantage(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        advantages = token_level_rewards * reward_mask * loss_weights
        returns = advantages.clone()
