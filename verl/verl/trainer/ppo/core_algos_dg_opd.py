...
def _dgopd_get_config(config):
    raw = _config_get(config, "dg_opd", {}) or {}
    return {
        "enabled": bool(_config_get(raw, "enabled", True)),
        "accuracy_threshold": float(_config_get(raw, "accuracy_threshold", 0.5)),
        "soft_gate": bool(_config_get(raw, "soft_gate", False)),
        "transition_width": max(1e-6, float(_config_get(raw, "transition_width", 0.25))),
        "opd_weight": float(_config_get(raw, "opd_weight", 1.0)),
        "rl_weight": float(_config_get(raw, "rl_weight", 1.0)),
        "norm_rl_adv_by_std": bool(_config_get(raw, "norm_rl_adv_by_std", True)),
...
def _dgopd_token_scores(token_level_rewards):
    if token_level_rewards.dim() == 3:
...
def _dgopd_smooth(values, response_mask, window_size):
    if window_size <= 1:
...
def _dgopd_causal_prefix_mean(values, response_mask, window_size):
...
def _dgopd_find_horizon(reliability, response_mask, ah_cfg):
...
def _dgopd_reliability_weights(
    token_level_rewards,
    response_mask,
    config,
    overlap_mask=None,
    ...
    ah_cfg = _dgopd_get_config(config)
    ...
    prefix_score = _dgopd_causal_prefix_mean(overlap_score, mask, ah_cfg["prefix_window_size"]).clamp(0.0, 1.0)
    ...
    reliability = _dgopd_smooth(reliability, mask, ah_cfg["window_size"]) * mask

    horizons = _dgopd_find_horizon(reliability, mask, ah_cfg)
    ...
    components = {
        "dgopd_overlap_score": overlap_score.detach(),
        "dgopd_gap_score": gap_score.detach(),
        "dgopd_prefix_score": prefix_score.detach(),
    }
...
def compute_adaptive_horizon_token_reward_advantage(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        token_weights, reliability, horizons, reliability_components = _dgopd_reliability_weights(
    ...
    extra_metrics = {
        "dgopd_token_weights": token_weights.detach(),
        "dgopd_reliability": reliability.detach(),
        "dgopd_horizon": horizons.to(dtype=token_level_rewards.dtype).detach(),
        "dgopd_outcome_weights": ((1.0 - token_weights) * response_mask.to(dtype=token_weights.dtype)).detach(),
    }
    if outcome_advantages is not None:
        extra_metrics["dgopd_outcome_advantages"] = outcome_advantages.detach()
    extra_metrics.update(reliability_components)
    return advantages, returns, extra_metrics


@register_adv_est("difficulty_gated_opd")
def compute_difficulty_gated_opd_advantage(
    token_level_rewards: torch.Tensor,
    response_mask: torch.Tensor,
    config: Optional[AlgoConfig] = None,
    ...
    **kwargs,
    ...
    Estimate prompt difficulty from the current on-policy group accuracy. Low
    accuracy groups use teacher dense OPD rewards; high accuracy groups use RL
    outcome advantages from rule rewards.
    """
    with torch.no_grad():
        dg_cfg = _dgopd_get_config(config)
        if token_level_rewards.dim() == 3:
            reward_mask = response_mask.unsqueeze(-1).to(dtype=token_level_rewards.dtype)
        else:
            reward_mask = response_mask.to(dtype=token_level_rewards.dtype)
        ...
        if index is None:
            # Fall back to per-sample difficulty if the trainer does not provide
            # repeated prompt ids. Normal training provides uid groups.
            group_accuracy = outcome_scores.clamp(0.0, 1.0)
        else:
            id2scores = defaultdict(list)
        ...
        if dg_cfg["soft_gate"]:
            rl_sample_weight = torch.sigmoid(
                (group_accuracy - dg_cfg["accuracy_threshold"]) / dg_cfg["transition_width"]
            )
        else:
            rl_sample_weight = (group_accuracy >= dg_cfg["accuracy_threshold"]).to(dtype=token_level_rewards.dtype)
        opd_sample_weight = 1.0 - rl_sample_weight

        if index is not None:
            rl_advantages, _ = compute_grpo_outcome_advantage(
                token_level_rewards=rewards_for_outcome,
                response_mask=response_mask,
                index=index,
                norm_adv_by_std_in_grpo=dg_cfg["norm_rl_adv_by_std"],
                config=config,
            )
        else:
            centered_scores = outcome_scores - outcome_scores.mean()
            if dg_cfg["norm_rl_adv_by_std"]:
                centered_scores = centered_scores / outcome_scores.std().clamp_min(1e-6)
            rl_advantages = centered_scores.unsqueeze(-1) * response_mask.to(dtype=token_level_rewards.dtype)

        opd_weights = opd_sample_weight.unsqueeze(-1) * response_mask.to(dtype=token_level_rewards.dtype)
        rl_weights = rl_sample_weight.unsqueeze(-1) * response_mask.to(dtype=token_level_rewards.dtype)
        if token_level_rewards.dim() == 3:
            opd_weights_for_adv = opd_weights.unsqueeze(-1)
            rl_weights_for_adv = rl_weights.unsqueeze(-1)
            rl_advantages_for_adv = rl_advantages.unsqueeze(-1)
        else:
            opd_weights_for_adv = opd_weights
    ...
    extra_metrics = {
        "dgopd_group_accuracy": group_accuracy.detach(),
