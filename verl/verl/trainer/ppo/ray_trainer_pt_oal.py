...
from verl.trainer.ppo import core_algos_pt_oal as core_algos
from verl.trainer.ppo.core_algos_pt_oal import AdvantageEstimator, agg_loss
...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if "teacher_entropy" in data.batch:  # optional teacher uncertainty diagnostic
            adv_kwargs["teacher_entropy"] = data.batch["teacher_entropy"]
        if "teacher_sampled_log_probs" in data.batch:  # required by PT-OAL prefix trust
            adv_kwargs["teacher_sampled_log_probs"] = data.batch["teacher_sampled_log_probs"]
        if "teacher_candidate_log_probs" in data.batch:  # top-k teacher support for aligned boost
            adv_kwargs["teacher_candidate_log_probs"] = data.batch["teacher_candidate_log_probs"]
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.use_rm and "rm_scores" not in batch.batch.keys():
                            ...
                            topk_renormalize = self.config.algorithm.get("pt_oal", {}).get("topk_renormalize", True)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if "oal_keep_mask" in batch.batch.keys():
                            ...
                            if "pt_oal_prefix_weights" in batch.batch.keys():
                                ...
                                suffix_mask = (
                                    (positions_pt >= prefix_horizon.long().unsqueeze(-1)).float()
                                    * response_mask.float()
                                    * prefix_triggered.unsqueeze(-1)
                                )
                                suffix_count = suffix_mask.sum().clamp_min(1)
                                metrics["pt_oal/suffix_weight_mean"] = (
