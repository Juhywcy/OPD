...
def _oal_get_config(config):
    raw = _config_get(config, "oal_opd", {}) or {}
    return {
        "enabled": bool(_config_get(raw, "enabled", True)),
...
def _boal_get_config(config):
    raw = _config_get(config, "boal_opd", {}) or {}
    return {
        "enabled": bool(_config_get(raw, "enabled", True)),
        "block_size": max(1, int(_config_get(raw, "block_size", 512))),
        "align_margin": float(_config_get(raw, "align_margin", 0.0)),
        "decay_lambda": max(0.0, float(_config_get(raw, "decay_lambda", 0.25))),
        "eps": max(1e-8, float(_config_get(raw, "eps", 1e-6))),
    }
...
def compute_block_outcome_aligned_opd_advantage(
    ...
    **kwargs,
    ...
    """
    Block-Causal Outcome-Aligned OPD.

    Split each response into fixed-length blocks. For each block, sum the
    teacher-minus-student OPD rewards as a block-level discriminative score.
    A block is aligned when this score is positive for a correct response, or
    negative for a wrong response. Future block weights decay with the number
    of previous misaligned blocks.
    """
    with torch.no_grad():
        boal_cfg = _boal_get_config(config)
        if token_level_rewards.dim() == 3:
            reward_mask = response_mask.unsqueeze(-1).to(dtype=token_level_rewards.dtype)
        else:
            reward_mask = response_mask.to(dtype=token_level_rewards.dtype)
        ...
        dense_advantages = token_level_rewards * reward_mask
        if not boal_cfg["enabled"]:
            returns = dense_advantages.clone()
            return dense_advantages, returns, {}

        mask = response_mask.to(dtype=token_level_rewards.dtype)
        align_scores = kwargs.get("logit_delta_scores", token_level_rewards).to(dtype=token_level_rewards.dtype)
        token_scores = _ahopd_token_scores(align_scores).to(dtype=token_level_rewards.dtype)
        token_scores = token_scores * mask
        outcome_scores = _odw_outcome_scores(kwargs.get("true_reward_score", None), token_level_rewards, response_mask)
        correct = outcome_scores > 0.5

        batch_size, seq_len = response_mask.shape
        block_size = boal_cfg["block_size"]
        block_weights = torch.zeros_like(mask)
        block_scores = torch.zeros_like(mask)
        block_aligned = torch.zeros_like(mask)
        cumulative_bad = torch.zeros_like(mask)

        for batch_idx in range(batch_size):
            previous_bad = 0.0
            is_correct = bool(correct[batch_idx].item())

            for start in range(0, seq_len, block_size):
                end = min(start + block_size, seq_len)
                block_mask = mask[batch_idx, start:end]
                if not bool((block_mask > 0).any().item()):
                    continue

                score = token_scores[batch_idx, start:end].sum()
                if is_correct:
                    aligned = score > boal_cfg["align_margin"]
                else:
                    aligned = score < -boal_cfg["align_margin"]

                current_bad = 0.0 if bool(aligned.item()) else 1.0
                bad_count = previous_bad + current_bad

                weight = torch.exp(
                    torch.tensor(
                        -boal_cfg["decay_lambda"] * bad_count,
                        device=mask.device,
                        dtype=mask.dtype,
                    )
                )

                block_weights[batch_idx, start:end] = weight
                block_scores[batch_idx, start:end] = score
                block_aligned[batch_idx, start:end] = float(bool(aligned.item()))
                cumulative_bad[batch_idx, start:end] = bad_count

                previous_bad = bad_count

        block_weights = block_weights * mask
        if token_level_rewards.dim() == 3:
            advantages = dense_advantages * block_weights.unsqueeze(-1)
        else:
            advantages = dense_advantages * block_weights
        returns = advantages.clone()

    extra_metrics = {
        "boal_block_weights": block_weights.detach(),
        "boal_block_scores": block_scores.detach(),
        "boal_block_aligned": block_aligned.detach(),
        "boal_cumulative_bad": cumulative_bad.detach(),
        "boal_outcome_scores": outcome_scores.detach(),
        "boal_correct_mask": correct.to(dtype=mask.dtype).detach(),
    }
    return advantages, returns, extra_metrics
