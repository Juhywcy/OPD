...
from verl.trainer.ppo import core_algos_boal_opd as core_algos
from verl.trainer.ppo.core_algos_boal_opd import AdvantageEstimator, agg_loss
...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if "overlap_mask" in data.batch:  # optional OPD diagnostic for AH-OPD
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("reward", timing_raw, color="yellow"):
                        ...
                        if self.use_rm and "rm_scores" not in batch.batch.keys():
                            ...
                            topk_renormalize = self.config.algorithm.get("boal_opd", {}).get("topk_renormalize", True)
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if "daoa_horizon" in batch.batch.keys():
                            response_mask = batch.batch["response_mask"]
                            valid_tokens = response_mask.sum().clamp_min(1)
                            response_lens = response_mask.sum(dim=-1).float().clamp_min(1.0)
                            daoa_horizon = batch.batch["daoa_horizon"].float()
                            daoa_prefix_ratio = (daoa_horizon / response_lens).clamp(0.0, 1.0)
                            metrics["daoa/horizon_mean"] = daoa_horizon.mean().item()
                            metrics["daoa/prefix_ratio_mean"] = daoa_prefix_ratio.mean().item()
                            metrics["daoa/suffix_ratio_mean"] = (1.0 - daoa_prefix_ratio).mean().item()
                            metrics["daoa/reliability_mean"] = (
                                (batch.batch["daoa_reliability"] * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["daoa/transition_alpha_mean"] = (
                                (batch.batch["daoa_transition_alpha"] * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["daoa/outcome_weight_mean"] = (
                                (batch.batch["daoa_outcome_weights"] * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["daoa/group_accuracy_mean"] = batch.batch["daoa_group_accuracy"].float().mean().item()
                            metrics["daoa/group_accuracy_min"] = batch.batch["daoa_group_accuracy"].float().min().item()
                            metrics["daoa/group_accuracy_max"] = batch.batch["daoa_group_accuracy"].float().max().item()
                            metrics["daoa/difficulty_lambda_mean"] = batch.batch["daoa_difficulty_lambda"].float().mean().item()
                            metrics["daoa/difficulty_lambda_min"] = batch.batch["daoa_difficulty_lambda"].float().min().item()
                            metrics["daoa/difficulty_lambda_max"] = batch.batch["daoa_difficulty_lambda"].float().max().item()
                            metrics["daoa/outcome_score_mean"] = batch.batch["daoa_outcome_scores"].float().mean().item()
                            weighted_outcome = batch.batch["daoa_outcome_advantages"] * batch.batch["daoa_outcome_weights"]
                            metrics["daoa/weighted_outcome_advantage_mean"] = (
                                (weighted_outcome * response_mask).sum() / valid_tokens
                            ).item()
 

                        if "odw_window_weights" in batch.batch.keys():
                            response_mask = batch.batch["response_mask"]
                            valid_tokens = response_mask.sum().clamp_min(1)
                            window_weights = batch.batch["odw_window_weights"].float()
                            window_deltas = batch.batch["odw_window_deltas"].float()
                            valid_window_weights = window_weights[response_mask > 0]
                            metrics["odw/window_weight_mean"] = (
                                (window_weights * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["odw/window_weight_min"] = valid_window_weights.min().item()
                            metrics["odw/window_weight_max"] = valid_window_weights.max().item()
                            metrics["odw/window_delta_mean"] = (
                                (window_deltas * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["odw/kept_sample_ratio"] = batch.batch["odw_group_keep"].float().mean().item()
                            metrics["odw/mixed_group_ratio"] = batch.batch["odw_mixed_group"].float().mean().item()
                            metrics["odw/all_correct_group_ratio"] = (
                                batch.batch["odw_all_correct_group"].float().mean().item()
                            )
                            metrics["odw/all_wrong_group_ratio"] = batch.batch["odw_all_wrong_group"].float().mean().item()
                            metrics["odw/outcome_score_mean"] = batch.batch["odw_outcome_scores"].float().mean().item()
                            metrics["odw/positive_sample_ratio"] = batch.batch["odw_positive_mask"].float().mean().item()
                            metrics["odw/active_token_ratio"] = (
                                batch.batch["odw_loss_mask"].float().sum() / valid_tokens
                            ).item()

                        if "oal_keep_mask" in batch.batch.keys():
                            response_mask = batch.batch["response_mask"]
                        ...
                        if "boal_block_weights" in batch.batch.keys():
                            response_mask = batch.batch["response_mask"]
                            valid_tokens = response_mask.sum().clamp_min(1)
                            block_weights = batch.batch["boal_block_weights"].float()
                            block_scores = batch.batch["boal_block_scores"].float()
                            block_aligned = batch.batch["boal_block_aligned"].float()
                            cumulative_bad = batch.batch["boal_cumulative_bad"].float()
                            valid_block_weights = block_weights[response_mask > 0]
                            metrics["boal/block_weight_mean"] = (
                                (block_weights * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["boal/block_weight_min"] = valid_block_weights.min().item()
                            metrics["boal/block_weight_max"] = valid_block_weights.max().item()
                            metrics["boal/block_score_mean"] = (
                                (block_scores * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["boal/block_score_abs_mean"] = (
                                (block_scores.abs() * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["boal/aligned_token_ratio"] = (
                                (block_aligned * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["boal/cumulative_bad_mean"] = (
                                (cumulative_bad * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["boal/correct_response_ratio"] = batch.batch["boal_correct_mask"].float().mean().item()
                            metrics["boal/outcome_score_mean"] = batch.batch["boal_outcome_scores"].float().mean().item()
                    ...
                    keys_to_pop = [
                        ...
                        "daoa_horizon",
                        "daoa_reliability",
                        "daoa_transition_alpha",
                        "daoa_group_accuracy",
                        "daoa_difficulty_lambda",
                        "daoa_outcome_scores",
                        "daoa_outcome_weights",
                        "daoa_outcome_advantages",
                        "daoa_teacher_weights",
                        "odw_window_weights",
                        "odw_window_deltas",
                        "odw_group_keep",
                        "odw_mixed_group",
                        "odw_all_correct_group",
                        "odw_all_wrong_group",
                        "odw_outcome_scores",
                        "odw_positive_mask",
                        "odw_negative_mask",
