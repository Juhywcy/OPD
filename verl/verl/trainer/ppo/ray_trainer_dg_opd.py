...
from verl.trainer.ppo import core_algos_dg_opd as core_algos
from verl.trainer.ppo.core_algos_dg_opd import AdvantageEstimator, agg_loss
...
def compute_advantage(
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        if "overlap_mask" in data.batch:  # optional OPD diagnostic for DG-OPD
            adv_kwargs["overlap_mask"] = data.batch["overlap_mask"]
        if "teacher_entropy" in data.batch:  # optional OPD diagnostic for DG-OPD
            adv_kwargs["teacher_entropy"] = data.batch["teacher_entropy"]
        if "student_entropy" in data.batch:  # optional OPD diagnostic for DG-OPD
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("adv", timing_raw, color="brown"):
                        ...
                        if "dgopd_horizon" in batch.batch.keys():
                            response_mask = batch.batch["response_mask"]
                            valid_tokens = response_mask.sum().clamp_min(1)
                            response_lens = response_mask.sum(dim=-1).float().clamp_min(1.0)
                            dgopd_horizon = batch.batch["dgopd_horizon"].float()
                            prefix_ratio = (dgopd_horizon / response_lens).clamp(0.0, 1.0)
                            metrics["dgopd/horizon_mean"] = dgopd_horizon.mean().item()
                            metrics["dgopd/horizon_min"] = dgopd_horizon.min().item()
                            metrics["dgopd/horizon_max"] = dgopd_horizon.max().item()
                            metrics["dgopd/prefix_ratio_mean"] = prefix_ratio.mean().item()
                            metrics["dgopd/suffix_ratio_mean"] = (1.0 - prefix_ratio).mean().item()
                            metrics["dgopd/reliability_mean"] = (
                                (batch.batch["dgopd_reliability"] * response_mask).sum() / valid_tokens
                            ).item()
                            metrics["dgopd/token_weight_mean"] = (
                                (batch.batch["dgopd_token_weights"] * response_mask).sum() / valid_tokens
                            ).item()
                            if "dgopd_outcome_weights" in batch.batch.keys():
                                metrics["dgopd/outcome_weight_mean"] = (
                                    (batch.batch["dgopd_outcome_weights"] * response_mask).sum() / valid_tokens
                                ).item()
                            if "dgopd_outcome_advantages" in batch.batch.keys():
                                outcome_advantages = batch.batch["dgopd_outcome_advantages"]
                                ...
                                metrics["dgopd/outcome_advantage_mean"] = (
                                    (outcome_advantages * outcome_mask).sum() / outcome_valid_tokens
                                ).item()
                            for key, metric_name in [
                                ("dgopd_overlap_score", "dgopd/overlap_score_mean"),
                                ("dgopd_gap_score", "dgopd/gap_score_mean"),
                                ("dgopd_prefix_score", "dgopd/prefix_score_mean"),
                            ]:
                                if key in batch.batch.keys():
                                    metrics[metric_name] = ((batch.batch[key] * response_mask).sum() / valid_tokens).item()
 
                        if "dgopd_group_accuracy" in batch.batch.keys():
                            response_mask = batch.batch["response_mask"]
                            valid_tokens = response_mask.sum().clamp_min(1)
                            ...
                            metrics["dgopd/hard_group_ratio"] = (group_accuracy < self.config.algorithm.dg_opd.accuracy_threshold).float().mean().item()
                            metrics["dgopd/easy_group_ratio"] = (group_accuracy >= self.config.algorithm.dg_opd.accuracy_threshold).float().mean().item()
                            if "dgopd_outcome_scores" in batch.batch.keys():
                                outcome_scores = batch.batch["dgopd_outcome_scores"].float()
                                metrics["dgopd/outcome_score_mean"] = outcome_scores.mean().item()
                                metrics["dgopd/outcome_score_min"] = outcome_scores.min().item()
                                metrics["dgopd/outcome_score_max"] = outcome_scores.max().item()
                            if "dgopd_opd_weights" in batch.batch.keys():
                                metrics["dgopd/opd_weight_mean"] = (
                                    (batch.batch["dgopd_opd_weights"] * response_mask).sum() / valid_tokens
                                ).item()
                            if "dgopd_rl_weights" in batch.batch.keys():
                            ...
                            if "dgopd_rl_advantages" in batch.batch.keys():
                                ...
                                metrics["dgopd/rl_advantage_mean"] = (
                                    (rl_advantages * response_mask).sum() / valid_tokens
                                ).item()
                    ...
                    if self.config.trainer.get("is_plot", False) and (self.global_steps == 1 or self.global_steps % 10 == 0):
                        try:
                            ...
                            if "teacher_entropy" in batch.batch.keys():
                                ...
                                # DG-OPD diagnostic plots: reliability, token weights, and horizon distribution.
                                log_payload = {
                                ...
                                if (
                                    "dgopd_reliability" in batch.batch.keys()
                                    and "dgopd_token_weights" in batch.batch.keys()
                                ):
                                    reliability_cpu = batch.batch["dgopd_reliability"].detach().cpu()
                                    token_weight_cpu = batch.batch["dgopd_token_weights"].detach().cpu()
                                    if "dgopd_outcome_weights" in batch.batch.keys():
                                        outcome_weight_cpu = batch.batch["dgopd_outcome_weights"].detach().cpu()
                                    else:
                                        outcome_weight_cpu = (1.0 - batch.batch["dgopd_token_weights"]).detach().cpu()
                                    component_cpus = {}
                                    ...
                                    plt.title(f"DG-OPD Reliability vs Position (Step {self.global_steps})")
                                    plt.xlabel("Position")
                                    plt.ylabel("Reliability")
                                    plt.ylim(-0.05, 1.05)
                                    plt.tight_layout()
                                    reliability_scatter_plot = swanlab.Image(
                                        plt, caption=f"DG-OPD Reliability vs Position (Step {self.global_steps})"
                                    )
                                    plt.close()

                                    plt.figure(figsize=(10, 6))
                                    plt.scatter(valid_positions, valid_token_weight, alpha=0.05, s=1)
                                    plt.title(f"DG-OPD Token Weight vs Position (Step {self.global_steps})")
                                    plt.xlabel("Position")
                                    plt.ylabel("Token Weight")
                                    plt.ylim(-0.05, 1.05)
                                    plt.tight_layout()
                                    token_weight_scatter_plot = swanlab.Image(
                                        plt, caption=f"DG-OPD Token Weight vs Position (Step {self.global_steps})"
                                    )
                                    plt.close()

                                    plt.figure(figsize=(10, 6))
                                    plt.scatter(valid_positions, valid_outcome_weight, alpha=0.05, s=1)
                                    plt.title(f"DG-OPD Outcome Weight vs Position (Step {self.global_steps})")
                                    plt.xlabel("Position")
                                    plt.ylabel("Outcome Weight")
                                    plt.ylim(-0.05, 1.05)
                                    plt.tight_layout()
                                    outcome_weight_scatter_plot = swanlab.Image(
                                        plt, caption=f"DG-OPD Outcome Weight vs Position (Step {self.global_steps})"
                                    )
                                    plt.close()
                                    ...
                                    plt.title(f"Avg DG-OPD Reliability vs Position (Step {self.global_steps})")
                                    plt.xlabel("Position")
                                    ...
                                    avg_reliability_plot = swanlab.Image(
                                        plt, caption=f"Avg DG-OPD Reliability vs Position (Step {self.global_steps})"
                                    )
                                    plt.close()

                                    plt.figure(figsize=(10, 6))
                                    plt.plot(plot_positions, plot_avg_token_weight)
                                    plt.title(f"Avg DG-OPD Token Weight vs Position (Step {self.global_steps})")
                                    plt.xlabel("Position")
                                    ...
                                    avg_token_weight_plot = swanlab.Image(
                                        plt, caption=f"Avg DG-OPD Token Weight vs Position (Step {self.global_steps})"
                                    )
                                    plt.close()

                                    plt.figure(figsize=(10, 6))
                                    plt.plot(plot_positions, plot_avg_outcome_weight)
                                    plt.title(f"Avg DG-OPD Outcome Weight vs Position (Step {self.global_steps})")
                                    plt.xlabel("Position")
                                    ...
                                    avg_outcome_weight_plot = swanlab.Image(
                                        plt, caption=f"Avg DG-OPD Outcome Weight vs Position (Step {self.global_steps})"
                                    )
                                    plt.close()

                                    log_payload.update(
                                        {
                                            "viz/dgopd_reliability_scatter": reliability_scatter_plot,
                                            "viz/dgopd_token_weight_scatter": token_weight_scatter_plot,
                                            "viz/dgopd_outcome_weight_scatter": outcome_weight_scatter_plot,
                                            "viz/avg_dgopd_reliability_line": avg_reliability_plot,
                                            "viz/avg_dgopd_token_weight_line": avg_token_weight_plot,
                                            "viz/avg_dgopd_outcome_weight_line": avg_outcome_weight_plot,
                                        }
                                    )

                                    component_plot_names = {
                                        "dgopd_overlap_score": ("viz/avg_dgopd_overlap_score_line", "Avg DG-OPD Overlap Score"),
                                        "dgopd_gap_score": ("viz/avg_dgopd_gap_score_line", "Avg DG-OPD Entropy Gap Score"),
                                        "dgopd_prefix_score": ("viz/avg_dgopd_prefix_score_line", "Avg DG-OPD Prefix Score"),
                                    }
                                    component_plots = []
                                    ...
                                    if "dgopd_horizon" in batch.batch.keys():
                                        save_payload["horizon"] = batch.batch["dgopd_horizon"].detach().float().cpu().numpy()
                                    np.savez_compressed(
                                ...
                                if "dgopd_horizon" in batch.batch.keys():
                                    horizon_cpu = batch.batch["dgopd_horizon"].detach().float().cpu().numpy()
                                    plt.figure(figsize=(10, 6))
                                    plt.hist(horizon_cpu, bins=30)
                                    plt.title(f"DG-OPD Horizon Distribution (Step {self.global_steps})")
                                    plt.xlabel("Horizon")
                                    plt.ylabel("Count")
                                    plt.grid(True)
                                    plt.tight_layout()
                                    horizon_hist_plot = swanlab.Image(
                                        plt, caption=f"DG-OPD Horizon Distribution (Step {self.global_steps})"
                                    )
                                    plt.close()
                                    log_payload["viz/dgopd_horizon_hist"] = horizon_hist_plot
                    ...
                    keys_to_pop = [
                        ...
                        "dgopd_gap_score",
                        "dgopd_prefix_score",
                        "dgopd_outcome_weights",
                        "dgopd_outcome_advantages",
                        "dgopd_group_accuracy",
