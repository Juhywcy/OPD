...
export ADV_ESTIMATOR=${ADV_ESTIMATOR:-difficulty_gated_opd}
...
export DG_OPD_ACCURACY_THRESHOLD=${DG_OPD_ACCURACY_THRESHOLD:-0.5}
export DG_OPD_SOFT_GATE=${DG_OPD_SOFT_GATE:-False}
export DG_OPD_TRANSITION_WIDTH=${DG_OPD_TRANSITION_WIDTH:-0.25}
export DG_OPD_OPD_WEIGHT=${DG_OPD_OPD_WEIGHT:-1.0}
export DG_OPD_RL_WEIGHT=${DG_OPD_RL_WEIGHT:-1.0}
export DG_OPD_NORM_RL_ADV_BY_STD=${DG_OPD_NORM_RL_ADV_BY_STD:-True}
# export LR=${LR:-1e-6}
...
export ACTOR_MODEL_PATH=${ACTOR_MODEL_PATH:-/home/models/Qwen/Qwen3-0.6B-Base}
...
export RUN_NAME=${RUN_NAME:-dgopd_${TRAIN_DATASET_NAME}_${ACTOR_MODEL_NAME}_${REWARD_MODEL_NAME}_${MAX_RESP_LENGTH}-n${N_RESPONSES}-mbs${MINI_BATCH_SIZE}-acc_thr${DG_OPD_ACCURACY_THRESHOLD}-soft${DG_OPD_SOFT_GATE}-opd${DG_OPD_OPD_WEIGHT}-rl${DG_OPD_RL_WEIGHT}-${MODEL_DTYPE}}
...
python3 -m verl.trainer.main_ppo_dg_opd \
    algorithm.adv_estimator=$ADV_ESTIMATOR \
    algorithm.grpo_outcome_weight=$GRPO_OUTCOME_WEIGHT \
    +algorithm.dg_opd.enabled=True \
    +algorithm.dg_opd.accuracy_threshold=$DG_OPD_ACCURACY_THRESHOLD \
    +algorithm.dg_opd.soft_gate=$DG_OPD_SOFT_GATE \
    +algorithm.dg_opd.transition_width=$DG_OPD_TRANSITION_WIDTH \
    +algorithm.dg_opd.opd_weight=$DG_OPD_OPD_WEIGHT \
    +algorithm.dg_opd.rl_weight=$DG_OPD_RL_WEIGHT \
    +algorithm.dg_opd.norm_rl_adv_by_std=$DG_OPD_NORM_RL_ADV_BY_STD \
    data.shuffle=False \
