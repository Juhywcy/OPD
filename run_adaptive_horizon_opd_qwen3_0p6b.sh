...
export ADV_ESTIMATOR=${ADV_ESTIMATOR:-adaptive_horizon_token_reward}
...
export AH_OPD_MIN_HORIZON=${AH_OPD_MIN_HORIZON:-1024}
export AH_OPD_WINDOW_SIZE=${AH_OPD_WINDOW_SIZE:-256}
export AH_OPD_THRESHOLD=${AH_OPD_THRESHOLD:-0.65}
export AH_OPD_PATIENCE_WINDOWS=${AH_OPD_PATIENCE_WINDOWS:-2}
export AH_OPD_SUFFIX_WEIGHT=${AH_OPD_SUFFIX_WEIGHT:-0.05}
...
export RUN_NAME=${RUN_NAME:-${ADV_ESTIMATOR}_reli_overlap_gap_prefix_${TRAIN_DATASET_NAME}_${ACTOR_MODEL_NAME}_${REWARD_MODEL_NAME}_${MAX_RESP_LENGTH}-T_${TEMPERATURE}-Tch_${TEACHER_TEMPERATURE}-n_${N_RESPONSES}-mbs_${MINI_BATCH_SIZE}-topk_${LOG_PROB_TOP_K}-topk_strategy_${TOP_K_STRATEGY}-rw_${REWARD_WEIGHT_MODE}-ah_min_${AH_OPD_MIN_HORIZON}-ah_win_${AH_OPD_WINDOW_SIZE}-ah_thr_${AH_OPD_THRESHOLD}-ah_pat_${AH_OPD_PATIENCE_WINDOWS}-ah_suf_${AH_OPD_SUFFIX_WEIGHT}-gap_tau_${AH_OPD_ENTROPY_GAP_TEMPERATURE}-prefix_win_${AH_OPD_PREFIX_WINDOW_SIZE}-w_${AH_OPD_OVERLAP_WEIGHT}_${AH_OPD_ENTROPY_WEIGHT}_${AH_OPD_REWARD_WEIGHT}-dtype_${MODEL_DTYPE}}
...
python3 -m verl.trainer.main_ppo_ah_opd \
    ...
    +algorithm.ah_opd.suffix_weight=$AH_OPD_SUFFIX_WEIGHT \
