...
export RUN_NAME=${RUN_NAME:-ahopd_relgap_${TRAIN_DATASET_NAME}_${ACTOR_MODEL_NAME}_${REWARD_MODEL_NAME}_${MAX_RESP_LENGTH}-n${N_RESPONSES}-mbs${MINI_BATCH_SIZE}-thr${AH_OPD_THRESHOLD}-pat${AH_OPD_PATIENCE_WINDOWS}-suf${AH_OPD_SUFFIX_WEIGHT}-gtau${AH_OPD_ENTROPY_GAP_TEMPERATURE}-pwin${AH_OPD_PREFIX_WINDOW_SIZE}-w${AH_OPD_OVERLAP_WEIGHT}_${AH_OPD_ENTROPY_WEIGHT}_${AH_OPD_REWARD_WEIGHT}-${MODEL_DTYPE}}
