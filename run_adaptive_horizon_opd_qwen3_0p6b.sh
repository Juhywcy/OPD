...
export AH_OPD_MIN_HORIZON=${AH_OPD_MIN_HORIZON:-4096}
...
export AH_OPD_OUTCOME_WEIGHT=${AH_OPD_OUTCOME_WEIGHT:-1.0}
# export LR=${LR:-1e-6}
...
export RUN_NAME=${RUN_NAME:-ahopd_mixout_${TRAIN_DATASET_NAME}_${ACTOR_MODEL_NAME}_${REWARD_MODEL_NAME}_${MAX_RESP_LENGTH}-n${N_RESPONSES}-mbs${MINI_BATCH_SIZE}-thr${AH_OPD_THRESHOLD}-pat${AH_OPD_PATIENCE_WINDOWS}-suf${AH_OPD_SUFFIX_WEIGHT}-out${AH_OPD_OUTCOME_WEIGHT}-gtau${AH_OPD_ENTROPY_GAP_TEMPERATURE}-pwin${AH_OPD_PREFIX_WINDOW_SIZE}-${MODEL_DTYPE}}
...
python3 -m verl.trainer.main_ppo_ah_opd \
    ...
    +algorithm.ah_opd.outcome_weight=$AH_OPD_OUTCOME_WEIGHT \
    data.shuffle=False \
