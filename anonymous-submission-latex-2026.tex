...
\title{Outcome-Aligned On-Policy Distillation with Rank-Calibrated Token Supervision}
\author{
...
\begin{itemize}
    \item We identify outcome-token mismatch as a practical failure mode of dense OPD rewards in long-horizon reasoning.
    \item We formulate outcome-aligned token supervision using the sign of teacher-student log-probability differences together with trajectory-level correctness.
    \item We introduce rank-calibrated OAL-OPD, which preserves aligned teacher supervision while softly down-weighting anti-aligned candidates using a bounded rank score.
    \item We provide a logging and analysis framework for studying how aligned and anti-aligned token candidates affect reasoning performance.
\end{itemize}
...
Let $\pi_S$ denote the student policy and $\pi_T$ denote the teacher policy. Given a prompt, the student samples a response $x_{1:T}$ from $\pi_S$. OPD computes a dense teacher reward for candidate token $a_{t,k}$ at position $t$ using the teacher-student log-probability difference:
\begin{equation}
\Delta_{t,k}
=
\log \pi_T(a_{t,k} \mid x_{<t})
-
\log \pi_S(a_{t,k} \mid x_{<t}).
\end{equation}
The candidate set can be either the sampled token itself or a top-$K$ candidate set. In the sampled-token setting, $K=1$ and $a_{t,1}=x_t$. In the top-$K$ setting, candidates are selected from the student distribution, the teacher distribution, or their union, depending on the implementation. A standard OPD objective directly uses $\Delta_{t,k}$ as a dense advantage-like signal:
\begin{equation}
A^{\mathrm{OPD}}_{t,k} = \Delta_{t,k}.
\end{equation}
The policy is then updated with a PPO-style objective using $A^{\mathrm{OPD}}_{t,k}$ as the token-level advantage. This treats all teacher preferences as equally reliable, regardless of whether they support or contradict the final correctness of the sampled response.

\subsection{Outcome-Aligned Token Partition}
...
{\cal B} = \{(t,k): s_{t,k}<-m\},
\end{equation}
where ${\cal A}$ contains outcome-aligned candidates and ${\cal B}$ contains outcome-anti-aligned candidates.

This partition has a direct interpretation. When $r=1$, aligned candidates are those whose probabilities the teacher would increase relative to the student. When $r=0$, aligned candidates are those whose probabilities the teacher would decrease relative to the student. Thus, the same rule handles both successful and failed trajectories without introducing a separate outcome reward into the optimization objective.

\subsection{Hard Outcome-Aligned OPD}

The most direct way to use the partition is to keep only aligned candidates:
\begin{equation}
w^{\mathrm{hard}}_{t,k}
=
{\bf 1}[s_{t,k}>m].
\end{equation}
The resulting dense advantage is
\begin{equation}
A^{\mathrm{hard}}_{t,k}
=
w^{\mathrm{hard}}_{t,k}\Delta_{t,k}.
\end{equation}
This hard variant removes teacher signals that conflict with the final outcome. It is simple and interpretable, but it may be overly brittle: a local teacher preference can be classified as anti-aligned because of reward noise, top-$K$ truncation, or uncertainty in the intermediate reasoning state. Completely discarding all anti-aligned candidates may therefore lose useful gradient information.

\subsection{Rank-Calibrated OAL-OPD}

OAL-OPD uses a softer treatment of anti-aligned candidates. The key design choice is asymmetric: aligned candidates are never down-weighted, while anti-aligned candidates are retained only with a small bounded weight. For each response, we rank only candidates in ${\cal B}$ according to their outcome-aligned score $s_{t,k}$. Let $n_i=|{\cal B}_i|$ be the number of anti-aligned candidates in response $i$. If $n_i>1$, the normalized rank is
\begin{equation}
q_{t,k}
=
\frac{\mathrm{rank}_{i}(s_{t,k})}{n_i-1},
\quad (t,k)\in{\cal B}_i,
\end{equation}
where the lowest anti-aligned score has rank $0$ and the largest anti-aligned score has rank $n_i-1$. If $n_i=1$, we set $q_{t,k}=1$. Candidates with relatively larger $s_{t,k}$ are closer to the decision boundary and receive larger rank values. The final weight is
\begin{equation}
w^{\mathrm{rank}}_{t,k}
=
\left\{
...
A^{\mathrm{rank}}_{t,k}
=
w^{\mathrm{rank}}_{t,k}\Delta_{t,k}.
\end{equation}
Unlike a rank scheme that rescales all candidates, OAL-OPD never down-weights outcome-aligned candidates. The ranking is applied only to the anti-aligned region, so $\beta$ has a clear meaning: it is the largest possible supervision strength assigned to candidates whose local teacher preference conflicts with the final outcome.

The hard variant is recovered when $\beta=0$. Standard OPD is approached when all candidates receive weight one. Therefore OAL-OPD interpolates between strict outcome filtering and dense OPD, but only through the anti-aligned region rather than through a global mixture coefficient.

\subsection{Position-Level Rank Calibration}

Candidate-level alignment can be too fine-grained when several top-$K$ candidates describe the same next-token decision. We therefore also consider a position-level variant that aggregates candidate-level evidence before assigning a weight:
\begin{equation}
s_t
=
y \sum_{k=1}^{K}\Delta_{t,k}.
\end{equation}
...
\right.
\end{equation}
where $q_t$ is the normalized rank among anti-aligned positions in the same response. This position-level weight is broadcast to all candidates at that position:
\begin{equation}
A^{\mathrm{pos}}_{t,k}
=
w^{\mathrm{pos}}_t\Delta_{t,k}.
\end{equation}
This variant tests whether outcome alignment should be judged at the individual candidate level or at the position level, where the teacher's top-$K$ distribution provides a more global view of the next-token decision.

\subsection{Training Objective}

After computing the OAL advantage $A_{t,k}$, we use the same policy optimization machinery as OPD. For top-$K$ candidates, the policy loss is computed over candidate log-probabilities:
\begin{equation}
{\cal L}_{\mathrm{OAL}}
=
-
\sum_{t=1}^{T}\sum_{k=1}^{K}
A_{t,k}\log \pi_S(a_{t,k}\mid x_{<t}).
\end{equation}
For sampled-token training, this reduces to the same expression with $K=1$. In practice, this objective is implemented inside a clipped PPO-style update, where $A_{t,k}$ replaces the original dense OPD advantage. OAL-OPD therefore changes only the token-level credit assignment, while keeping the rollout generation, teacher scoring, and actor update pipeline unchanged.

\subsection{Diagnostics}

The outcome-aligned partition also provides useful diagnostics. We track the candidate and token ratios of aligned and anti-aligned groups, the retained token weight, and the absolute advantage mass assigned to each group. These statistics allow us to determine whether a method improves performance by preserving aligned supervision, by suppressing harmful anti-aligned signals, or by retaining a small amount of anti-aligned information through rank calibration.
