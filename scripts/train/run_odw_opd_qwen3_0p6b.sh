...
export ADV_ESTIMATOR=${ADV_ESTIMATOR:-outcome_discriminative_window_opd}
...
export ODW_WINDOW_SIZE=${ODW_WINDOW_SIZE:-512}
export ODW_MARGIN_DELTA=${ODW_MARGIN_DELTA:-0.0}
export ODW_TEMPERATURE=${ODW_TEMPERATURE:-0.1}
export ODW_FILTER_MIXED=${ODW_FILTER_MIXED:-True}
# export LR=${LR:-1e-6}
...
export RUN_NAME=${RUN_NAME:-odw_opd_${TRAIN_DATASET_NAME}_${ACTOR_MODEL_NAME}_${REWARD_MODEL_NAME}_${MAX_RESP_LENGTH}-n${N_RESPONSES}-mbs${MINI_BATCH_SIZE}-win${ODW_WINDOW_SIZE}-md${ODW_MARGIN_DELTA}-tau${ODW_TEMPERATURE}-filter${ODW_FILTER_MIXED}-${MODEL_DTYPE}}
...
python3 -m verl.trainer.main_ppo_odw_opd \
    ...
    +algorithm.odw_opd.enabled=True \
    +algorithm.odw_opd.window_size=$ODW_WINDOW_SIZE \
    +algorithm.odw_opd.margin_delta=$ODW_MARGIN_DELTA \
    +algorithm.odw_opd.temperature=$ODW_TEMPERATURE \
    +algorithm.odw_opd.filter_mixed=$ODW_FILTER_MIXED \
    +algorithm.odw_opd.topk_renormalize=$OPD_TOPK_RENORMALIZE \
    data.shuffle=False \
