...
export ADV_ESTIMATOR=${ADV_ESTIMATOR:-block_outcome_aligned_opd}
...
export BOAL_BLOCK_SIZE=${BOAL_BLOCK_SIZE:-512}
export BOAL_ALIGN_MARGIN=${BOAL_ALIGN_MARGIN:-0.0}
export BOAL_DECAY_LAMBDA=${BOAL_DECAY_LAMBDA:-0.25}
# export LR=${LR:-1e-6}
...
export ACTOR_MODEL_PATH=${ACTOR_MODEL_PATH:-/home/models/deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B}
...
export REWARD_MODEL_PATH=${REWARD_MODEL_PATH:-/home/models/hbx/JustRL-DeepSeek-1.5B}
export REWARD_MODEL_NAME=$(basename "$REWARD_MODEL_PATH")

export PROJECT_PATH=checkpoint
export PARALLEL_SIZE=1
export RUN_NAME=${RUN_NAME:-boal_opd_${TRAIN_DATASET_NAME}_${ACTOR_MODEL_NAME}_${REWARD_MODEL_NAME}_${MAX_RESP_LENGTH}-n${N_RESPONSES}-mbs${MINI_BATCH_SIZE}-block${BOAL_BLOCK_SIZE}-margin${BOAL_ALIGN_MARGIN}-decay${BOAL_DECAY_LAMBDA}-${MODEL_DTYPE}}
...
python3 -m verl.trainer.main_ppo_boal_opd \
    algorithm.adv_estimator=$ADV_ESTIMATOR \
    algorithm.grpo_outcome_weight=$GRPO_OUTCOME_WEIGHT \
    +algorithm.boal_opd.enabled=True \
    +algorithm.boal_opd.block_size=$BOAL_BLOCK_SIZE \
    +algorithm.boal_opd.align_margin=$BOAL_ALIGN_MARGIN \
    +algorithm.boal_opd.decay_lambda=$BOAL_DECAY_LAMBDA \
    +algorithm.boal_opd.topk_renormalize=$OPD_TOPK_RENORMALIZE \
    data.shuffle=False \
